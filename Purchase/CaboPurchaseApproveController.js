(function () {
    ...
    .controller('CaboPurchaseApproveController', CaboPurchaseApproveController)
    .controller('CaboPurchaseApproveEditController', CaboPurchaseApproveEditController)

    CaboPurchaseApproveController.$inject = ['$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', '$q', 'CaboPurchaseApprove'];
    function CaboPurchaseApproveController($scope, $stateParams, $state, ngDialog, MyDialogs, toaster, $filter, $q, CaboPurchaseApprove) {
        ...
        var load = $scope.load = function () {
            ...
            CaboPurchaseApprove.get($scope.p, function (r) {
                $scope.list = r.Result;
        ...
        var edit = $scope.edit = function (ID) {
            if (ID)
                $state.go('app.Purchase.CaboPurchaseApprove.edit', { ID: ID });
            else {
                var items = [];
                ...
                $state.go('app.Purchase.CaboPurchaseApprove.edit', { ID: items[0] });
            }
        }
        ...
        var del = $scope.del = function () {
            MyDialogs.DeleteConfirm().then(function (value) {
                ...
                CaboPurchaseApprove.Deletes(IDs, function (r) {
                    if (r.Success) {
    ...
    CaboPurchaseApproveEditController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', 'editableOptions', 'editableThemes', '$q', 'SysReceiptConfig', 'CaboPurchaseApprove','DicWarehouse'];
    function CaboPurchaseApproveEditController($rootScope, $scope, $stateParams, $state, ngDialog,MyDialogs, toaster, $filter, editableOptions, editableThemes, $q, SysReceiptConfig, CaboPurchaseApprove, DicWarehouse) {
        $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
        $scope.d = { CaboPurchaseApproveDetail: [] };
        var load = $scope.load = function () {
            if ($stateParams.ID) {
                $scope.isLoading = true
                $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
                CaboPurchaseApprove.Get({ ID: $stateParams.ID }, function (r) {
                    $scope.d = r.Result;
                    $scope.isLoading = false;
                })
            } else {
                ...
                SysReceiptConfig.GetBillNO({ TypeTab: "CaboPurchaseApprove" }, function (r) {
        ...
        var save = $scope.save = function () {
            ...
            if ($scope.subIDs.IDs.length > 0)
                CaboPurchaseApprove.DeleteDetails($scope.subIDs, function (r) {
                    if (r.Success) {
                    }
                    $scope.isLoading = false;
                });
            CaboPurchaseApprove.Post($scope.d, function (r) {
                if (r.Success) {
                    ...
                    if ($stateParams.ID)
                        ...
                        $state.go('app.Purchase.CaboPurchaseApprove.edit', { ID: r.Result.ID });
                }
            }, function (err) {
        ...
        $scope.addItem = function () {
            ...
            $scope.d.CaboPurchaseApproveDetail.push($scope.inserted);
        };

        //新添加的从列表中直接移除，原数据库的记录则保存ID
        $scope.removeItem = function (index, ID) { //string Id, string FID
            $scope.d.CaboPurchaseApproveDetail.splice(index, 1);
            if (ID)
        ...
        $scope.check = function (status) {
            MyDialogs.Check().then(function (value) {
                $scope.isLoading = true
                CaboPurchaseApprove.Audit({ 'ID': $scope.d.ID, 'AuditStatus': parseInt($scope.d.AuditStatus) + parseInt(status), 'AuditOpinions': value }, function (r) {
                    if (r.Success) {
                        $scope.isLoading = false;
