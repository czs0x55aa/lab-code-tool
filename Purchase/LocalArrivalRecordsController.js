(function () {
    ...
    .controller('LocalArrivalRecordsController', LocalArrivalRecordsController)
    .controller('LocalArrivalRecordsEditController', LocalArrivalRecordsEditController)

    LocalArrivalRecordsController.$inject = ['$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', '$q', 'LocalArrivalRecords'];
    function LocalArrivalRecordsController($scope, $stateParams, $state, ngDialog, MyDialogs, toaster, $filter, $q, LocalArrivalRecords) {
        ...
        var load = $scope.load = function () {
            ...
            LocalArrivalRecords.get($scope.p, function (r) {
                $scope.list = r.Result;
        ...
        var edit = $scope.edit = function (ID) {
            if (ID)
                $state.go('app.Purchase.LocalArrivalRecords.edit', { ID: ID });
            else {
                var items = [];
                ...
                $state.go('app.Purchase.LocalArrivalRecords.edit', { ID: items[0] });
            }
        }
        ...
        var del = $scope.del = function () {
            MyDialogs.DeleteConfirm().then(function (value) {
                ...
                LocalArrivalRecords.Deletes(IDs, function (r) {
                    if (r.Success) {
    ...
    LocalArrivalRecordsEditController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', 'editableOptions', 'editableThemes', '$q', 'SysReceiptConfig', 'LocalArrivalRecords','DicWarehouse'];
    function LocalArrivalRecordsEditController($rootScope, $scope, $stateParams, $state, ngDialog,MyDialogs, toaster, $filter, editableOptions, editableThemes, $q, SysReceiptConfig, LocalArrivalRecords, DicWarehouse) {
        $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
        $scope.d = { LocalArrivalRecordsDetail: [] };
        var load = $scope.load = function () {
            if ($stateParams.ID) {
                $scope.isLoading = true
                $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
                LocalArrivalRecords.Get({ ID: $stateParams.ID }, function (r) {
                    $scope.d = r.Result;
                    $scope.isLoading = false;
                })
            } else {
                ...
                SysReceiptConfig.GetBillNO({ TypeTab: "LocalArrivalRecords" }, function (r) {
        ...
        var save = $scope.save = function () {
            ...
            if ($scope.subIDs.IDs.length > 0)
                LocalArrivalRecords.DeleteDetails($scope.subIDs, function (r) {
                    if (r.Success) {
                    }
                    $scope.isLoading = false;
                });
            LocalArrivalRecords.Post($scope.d, function (r) {
                if (r.Success) {
                    ...
                    if ($stateParams.ID)
                        ...
                        $state.go('app.Purchase.LocalArrivalRecords.edit', { ID: r.Result.ID });
                }
            }, function (err) {
        ...
        $scope.addItem = function () {
            ...
            $scope.d.LocalArrivalRecordsDetail.push($scope.inserted);
        };

        //新添加的从列表中直接移除，原数据库的记录则保存ID
        $scope.removeItem = function (index, ID) { //string Id, string FID
            $scope.d.LocalArrivalRecordsDetail.splice(index, 1);
            if (ID)
        ...
        $scope.check = function (status) {
            MyDialogs.Check().then(function (value) {
                $scope.isLoading = true
                LocalArrivalRecords.Audit({ 'ID': $scope.d.ID, 'AuditStatus': parseInt($scope.d.AuditStatus) + parseInt(status), 'AuditOpinions': value }, function (r) {
                    if (r.Success) {
                        $scope.isLoading = false;
