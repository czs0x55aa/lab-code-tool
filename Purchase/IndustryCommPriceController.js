(function () {
    ...
    .controller('IndustryCommPriceController', IndustryCommPriceController)
    .controller('IndustryCommPriceEditController', IndustryCommPriceEditController)

    IndustryCommPriceController.$inject = ['$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', '$q', 'IndustryCommPrice'];
    function IndustryCommPriceController($scope, $stateParams, $state, ngDialog, MyDialogs, toaster, $filter, $q, IndustryCommPrice) {
        ...
        var load = $scope.load = function () {
            ...
            IndustryCommPrice.get($scope.p, function (r) {
                $scope.list = r.Result;
        ...
        var edit = $scope.edit = function (ID) {
            if (ID)
                $state.go('app.Purchase.IndustryCommPrice.edit', { ID: ID });
            else {
                var items = [];
                ...
                $state.go('app.Purchase.IndustryCommPrice.edit', { ID: items[0] });
            }
        }
        ...
        var del = $scope.del = function () {
            MyDialogs.DeleteConfirm().then(function (value) {
                ...
                IndustryCommPrice.Deletes(IDs, function (r) {
                    if (r.Success) {
    ...
    IndustryCommPriceEditController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', 'editableOptions', 'editableThemes', '$q', 'SysReceiptConfig', 'IndustryCommPrice','DicWarehouse'];
    function IndustryCommPriceEditController($rootScope, $scope, $stateParams, $state, ngDialog,MyDialogs, toaster, $filter, editableOptions, editableThemes, $q, SysReceiptConfig, IndustryCommPrice, DicWarehouse) {
        $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
        $scope.d = { IndustryCommPriceDetail: [] };
        var load = $scope.load = function () {
            if ($stateParams.ID) {
                $scope.isLoading = true
                $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
                IndustryCommPrice.Get({ ID: $stateParams.ID }, function (r) {
                    $scope.d = r.Result;
                    $scope.isLoading = false;
                })
            } else {
                ...
                SysReceiptConfig.GetBillNO({ TypeTab: "IndustryCommPrice" }, function (r) {
        ...
        var save = $scope.save = function () {
            ...
            if ($scope.subIDs.IDs.length > 0)
                IndustryCommPrice.DeleteDetails($scope.subIDs, function (r) {
                    if (r.Success) {
                    }
                    $scope.isLoading = false;
                });
            IndustryCommPrice.Post($scope.d, function (r) {
                if (r.Success) {
                    ...
                    if ($stateParams.ID)
                        ...
                        $state.go('app.Purchase.IndustryCommPrice.edit', { ID: r.Result.ID });
                }
            }, function (err) {
        ...
        $scope.addItem = function () {
            ...
            $scope.d.IndustryCommPriceDetail.push($scope.inserted);
        };

        //新添加的从列表中直接移除，原数据库的记录则保存ID
        $scope.removeItem = function (index, ID) { //string Id, string FID
            $scope.d.IndustryCommPriceDetail.splice(index, 1);
            if (ID)
        ...
        $scope.check = function (status) {
            MyDialogs.Check().then(function (value) {
                $scope.isLoading = true
                IndustryCommPrice.Audit({ 'ID': $scope.d.ID, 'AuditStatus': parseInt($scope.d.AuditStatus) + parseInt(status), 'AuditOpinions': value }, function (r) {
                    if (r.Success) {
                        $scope.isLoading = false;
