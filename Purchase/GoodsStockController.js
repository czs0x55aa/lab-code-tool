(function () {
    ...
    .controller('GoodsStockEditController', GoodsStockEditController)

    GoodsStockController.$inject = ['$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', '$q', 'GoodsStock'];
    function GoodsStockController($scope, $stateParams, $state, ngDialog, MyDialogs, toaster, $filter, $q, GoodsStock) {
        ...
        var load = $scope.load = function () {
            ...
            GoodsStock.get($scope.p, function (r) {
                $scope.list = r.Result;
        ...
        var edit = $scope.edit = function (ID) {
            if (ID)
                $state.go('app.Purchase.GoodsStock.edit', { ID: ID });
            else {
                var items = [];
                ...
                $state.go('app.Purchase.GoodsStock.edit', { ID: items[0] });
            }
        }
        ...
        var del = $scope.del = function () {
            MyDialogs.DeleteConfirm().then(function (value) {
                ...
                GoodsStock.Deletes(IDs, function (r) {
                    if (r.Success) {
    ...
    GoodsStockEditController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', 'editableOptions', 'editableThemes', '$q', 'SysReceiptConfig', 'GoodsStock','DicWarehouse'];
    function GoodsStockEditController($rootScope, $scope, $stateParams, $state, ngDialog,MyDialogs, toaster, $filter, editableOptions, editableThemes, $q, SysReceiptConfig, GoodsStock, DicWarehouse) {
        $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
        $scope.d = { GoodsStockDetail: [] };
        var load = $scope.load = function () {
            if ($stateParams.ID) {
                $scope.isLoading = true
                $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
                GoodsStock.Get({ ID: $stateParams.ID }, function (r) {
                    $scope.d = r.Result;
                    $scope.isLoading = false;
                })
            } else {
                ...
                SysReceiptConfig.GetBillNO({ TypeTab: "GoodsStock" }, function (r) {
        ...
        var save = $scope.save = function () {
            ...
            if ($scope.subIDs.IDs.length > 0)
                GoodsStock.DeleteDetails($scope.subIDs, function (r) {
                    if (r.Success) {
                    }
                    $scope.isLoading = false;
                });
            GoodsStock.Post($scope.d, function (r) {
                if (r.Success) {
                    ...
                    if ($stateParams.ID)
                        ...
                        $state.go('app.Purchase.GoodsStock.edit', { ID: r.Result.ID });
                }
            }, function (err) {
        ...
        $scope.addItem = function () {
            ...
            $scope.d.GoodsStockDetail.push($scope.inserted);
        };

        //新添加的从列表中直接移除，原数据库的记录则保存ID
        $scope.removeItem = function (index, ID) { //string Id, string FID
            $scope.d.GoodsStockDetail.splice(index, 1);
            if (ID)
        ...
        $scope.check = function (status) {
            MyDialogs.Check().then(function (value) {
                $scope.isLoading = true
                GoodsStock.Audit({ 'ID': $scope.d.ID, 'AuditStatus': parseInt($scope.d.AuditStatus) + parseInt(status), 'AuditOpinions': value }, function (r) {
                    if (r.Success) {
                        $scope.isLoading = false;
