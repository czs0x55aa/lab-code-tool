(function () {
    ...
    .controller('EstimateInInventoryController', EstimateInInventoryController)
    .controller('EstimateInInventoryEditController', EstimateInInventoryEditController)

    EstimateInInventoryController.$inject = ['$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', '$q', 'EstimateInInventory'];
    function EstimateInInventoryController($scope, $stateParams, $state, ngDialog, MyDialogs, toaster, $filter, $q, EstimateInInventory) {
        ...
        var load = $scope.load = function () {
            ...
            EstimateInInventory.get($scope.p, function (r) {
                $scope.list = r.Result;
        ...
        var edit = $scope.edit = function (ID) {
            if (ID)
                $state.go('app.Purchase.EstimateInInventory.edit', { ID: ID });
            else {
                var items = [];
                ...
                $state.go('app.Purchase.EstimateInInventory.edit', { ID: items[0] });
            }
        }
        ...
        var del = $scope.del = function () {
            MyDialogs.DeleteConfirm().then(function (value) {
                ...
                EstimateInInventory.Deletes(IDs, function (r) {
                    if (r.Success) {
    ...
    EstimateInInventoryEditController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', 'ngDialog','MyDialogs', 'toaster', '$filter', 'editableOptions', 'editableThemes', '$q', 'SysReceiptConfig', 'EstimateInInventory','DicWarehouse'];
    function EstimateInInventoryEditController($rootScope, $scope, $stateParams, $state, ngDialog,MyDialogs, toaster, $filter, editableOptions, editableThemes, $q, SysReceiptConfig, EstimateInInventory, DicWarehouse) {
        $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
        $scope.d = { EstimateInInventoryDetail: [] };
        var load = $scope.load = function () {
            if ($stateParams.ID) {
                $scope.isLoading = true
                $scope.subIDs = { "IDs": [], "FID": $stateParams.ID };
                EstimateInInventory.Get({ ID: $stateParams.ID }, function (r) {
                    $scope.d = r.Result;
                    $scope.isLoading = false;
                })
            } else {
                ...
                SysReceiptConfig.GetBillNO({ TypeTab: "EstimateInInventory" }, function (r) {
        ...
        var save = $scope.save = function () {
            ...
            if ($scope.subIDs.IDs.length > 0)
                EstimateInInventory.DeleteDetails($scope.subIDs, function (r) {
                    if (r.Success) {
                    }
                    $scope.isLoading = false;
                });
            EstimateInInventory.Post($scope.d, function (r) {
                if (r.Success) {
                    ...
                    if ($stateParams.ID)
                        ...
                        $state.go('app.Purchase.EstimateInInventory.edit', { ID: r.Result.ID });
                }
            }, function (err) {
        ...
        $scope.addItem = function () {
            ...
            $scope.d.EstimateInInventoryDetail.push($scope.inserted);
        };

        //新添加的从列表中直接移除，原数据库的记录则保存ID
        $scope.removeItem = function (index, ID) { //string Id, string FID
            $scope.d.EstimateInInventoryDetail.splice(index, 1);
            if (ID)
        ...
        $scope.check = function (status) {
            MyDialogs.Check().then(function (value) {
                $scope.isLoading = true
                EstimateInInventory.Audit({ 'ID': $scope.d.ID, 'AuditStatus': parseInt($scope.d.AuditStatus) + parseInt(status), 'AuditOpinions': value }, function (r) {
                    if (r.Success) {
                        $scope.isLoading = false;
